...
{
	fprintf(stderr, "Usage: ... bundle_inbox limit BYTES burst BYTES[/BYTES] rate KBPS [ mtu BYTES[/BYTES] ]\n");
...
static int tbf_parse_opt(struct qdisc_util *qu, int argc, char **argv, struct nlmsghdr *n)
{
	...
	if (tc_calc_rtable(&opt.rate, rtab, Rcell_log, mtu, linklayer) < 0) {
		fprintf(stderr, "bundle_inbox: failed to calculate rate table.\n");
		return -1;
	}
	...
	if (opt.peakrate.rate) {
		...
		if (tc_calc_rtable(&opt.peakrate, ptab, Pcell_log, mtu, linklayer) < 0) {
			fprintf(stderr, "bundle_inbox: failed to calculate peak rate table.\n");
			return -1;
		}
...
struct qdisc_util bundle_inbox_qdisc_util = {
	.id		= "bundle_inbox",
